                    ...
                    try
                    {
                        ...
                        if (folderResult.WasRateLimited)
                        {
                            ...
                            _logger.LogWarning("Rate limit hit during folder {FolderName} for account {AccountName}. " +
                                "Stopping folder iteration so the next run can resume cleanly.", folder.FullName, account.Name);
                            break;
                        }
                ...
                if (wasRateLimited)
                {
                    ...
                    if (jobId != null)
                    {
                        _syncJobService.CompleteJobRateLimited(jobId,
                            $"Rate limit reached during sync. Processed: {processedEmails}, New: {newEmails}. Sync will resume from checkpoint.");
                    }
                    return;
                }
                    ...
                    for (int i = 0; i < uids.Count; i += _batchOptions.BatchSize)
                    {
                        ...
                        foreach (var uid in batch)
                        {
                            ...
                            try
                            {
                                ...
                                using var messageToDispose = message;
